...
def inscribir_viatge(id: int, db: Session = Depends(get_db), user: models_usuari.Usuari = Depends(get_current_user)):
    resultado = crud.insribir_viatge(db, user, id)
...
def peticio_promocio(missatge: str = Body(..., embed=True), db: Session = Depends(get_db), user: models_usuari.Usuari = Depends(get_current_user)):
